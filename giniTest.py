...
#Calculate the GINI index for labels provided
def giniCalc(n, labels):
    gini = 0

    for label in labels.keys():
        gini += (labels[label] / n) ** 2
...
#Calculate the average GINI index for the remaining attrs and as well as partitioning the data based on the target attribute provided
def avg_gini_w_partitions(data, remaining_attrs, target):
    ...
    for partition_key in partitions.keys():
        ...
        partition_entropy = giniCalc(partition_n, partition_labels)
        avg_gini += partition_n / n * partition_entropy

    return avg_gini, partitions
...
def id3(data, uniqs, attrs, target):
    ...
    for a in attrs:
        avg_gini, partitions = avg_gini_w_partitions(data, a, target)
        info_gain = gini - avg_gini
