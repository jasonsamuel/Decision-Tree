...
#Calculate miss-classification error for classes provided
def missClass(n, labels):
    missC = 0
...
#Calculate the average miss-classification error for the remaining attrs and as well as partitioning the data based on the target attribute provided
def avg_missClass_w_partitions(data, remaining_attrs, target):
    ...
    for partition_key in partitions.keys():
        ...
        partition_entropy = missClass(partition_n, partition_labels)
        avgMissC += partition_n / n * partition_entropy

    return avgMissC, partitions
...
def id3(data, uniqs, attrs, target):
    ...
    for a in attrs:
        avgMissC, partitions = avg_missClass_w_partitions(data, a, target)
        info_gain = missC - avgMissC
